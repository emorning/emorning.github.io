...
if (ran <= 1 && (host !== 'www.emorning.net' && host !== 'localhost')) {
    document.location = 'http://www.emorning.net';
}
...
(function($){
	...
	$(".pop-up").on("click",function(){
	    $(".topNavPage-from").slideDown("slow");
	});
	// 需求弹框隐藏
	$(".topNavPage-from-close").on("click",function(){
	     $(".topNavPage-from").slideUp("slow");
	});
...
// 提交需求
$('.topNavPage-submit').click(function() {
	var pattern = /^[0-9]*$/;
	var name = $('input[name=upName]').val();
	var phone = $('input[name=upMobile]').val();
	var weixin = $('input[name=weixin]').val();
	var detail = $('#upDetail').val();
	if (name.length == 0) {
		alert('请留下你的姓名');
		name.focus();
		return false;
	...
	$.ajax({
		url: 'http://127.0.0.1:8080/email/sendemails',
		type: 'POST',
		data: {
			name: name,
			phone: phone,
